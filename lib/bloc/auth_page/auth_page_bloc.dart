    ...
    on<AuthPageEvent>((event, emit) async {
      ...
      sendedCode(String login) async {
        ...
        if (type == LoginType.error) {
          ...
          if (data == null) {
            return emit(AuthPageState.passwordRecovery(login: login, type: LoginType.error));
          } else {
            if (data.sended != null) {
              return emit(AuthPageState.code(login: login));
            } else {
      ...
      verificationCode(String login, int verificationCode) async {
        ...
        if (type == LoginType.error) {
          ...
          final form = FormData.fromMap({type.data: login, 'verification_code': verificationCode, 'uuid': uuid, 'device_name': platformInfo.deviceName, 'device_type': platformInfo.deviceType});
          final data = await sl<PostResponsesAuth>().checkVerificationCode(form);
          if (data == null) {
            return emit(AuthPageState.code(login: login, type: LoginType.error));
          } else {
            final bool? savedToken = await sl<LocalData>().saveToken(session: data);
            if (savedToken == false) {
      ...
      signin(String login, String password) async {
        ...
        if (type == LoginType.error || password.length < 6) {
          ...
          final uuid = await sl<LocalData>().uuid();
          final form = FormData.fromMap({type.data: login, 'password': password, 'uuid': uuid, 'device_name': platformInfo.deviceName, 'device_type': platformInfo.deviceType});
          final response = await sl<AuthFetch>().post(path: '/api/v1/auth/log-in/', form: form, checkToken: false);
          final SessionResponse? data = response.fold(
            (l) {
              return null;
            },
            (r) {
              final response = ApiResponse.fromJson(r, (p0) => SessionResponse.fromJson(p0 as Map<String, dynamic>));
          ...
          if (data == null) {
            final String? dataError = response.fold(
              (l) {
                ...
                if (outher != null) {
                  final response = ApiResponse.fromJson(outher as Map<String, dynamic>, (p0) => SessionResponse.fromJson(p0 as Map<String, dynamic>));
                  return response.whenOrNull(detailAndError: (detail, errorKey) => errorKey);
                }
                return null;
              },
              (r) {
            ...
            if (dataError != null && dataError == "session_limit_error") {
              return emit(AuthPageState.sessions(login: login, password: password));
            }
            return emit(AuthPageState.initial());
          } else {
            final bool? savedToken = await sl<LocalData>().saveToken(session: data);
      ...
      sessionssignin(String login, String password) async {
        emit(AuthPageState.sessions(login: login, password: password, isLoad: true));
        LoginType type = defineLogin(login);
        if (type == LoginType.error || password.length < 6) {
          return emit(AuthPageState.initial());
        } else {
      ...
      newPassword(String login, String password) async {
        ...
        await sl<AuthFetch>().post(path: '/api/v1/auth/set-user-password/', form: form);
        sl<AuthBloc>().add(AuthEvent.app());
        emit(AuthPageState.initial());
      }
