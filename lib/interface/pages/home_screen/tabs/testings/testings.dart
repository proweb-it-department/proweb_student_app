...
class TestingsProvider extends StatelessWidget {
  ...
  const TestingsProvider({super.key, this.group, this.groupDetail});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => HomeTestingsBloc()
        ..add(
          HomeTestingsEvent.started(groupId: groupId!, limit: 50, offset: 0),
        ),
      child: TestingsViewData(group: group, groupDetail: groupDetail),
    );
  }
}
...
class TestingsViewData extends StatelessWidget {
  final MyGroupsItem? group;
  final GroupDetail? groupDetail;
  const TestingsViewData({super.key, this.group, this.groupDetail});
