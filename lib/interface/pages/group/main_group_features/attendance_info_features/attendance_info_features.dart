...
class AttendanceGroupInfoScreen extends StatelessWidget {
  final int groupId;
  const AttendanceGroupInfoScreen({super.key, @PathParam.inherit('id') required this.groupId});
  ...
  Widget build(BuildContext context) {
    ...
    return groupDetail.state.when(
      ...
      complited: (group, groupUser) {
        return AttendanceInfoFeatures(group: group, groupId: groupId, groupUser: groupUser);
      },
    );
  }
}
...
class AttendanceInfoFeatures extends StatelessWidget {
  ...
  const AttendanceInfoFeatures({super.key, required this.group, required this.groupUser, required this.groupId});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => AttendanceLessonBloc()..add(AttendanceLessonEvent.started(groupId: groupId, version: group.version)),
      child: BlocBuilder<AttendanceLessonBloc, AttendanceLessonState>(
        builder: (context, state) {
          return state.when(
            ...
            complited: (lessons, complitedLesson, attendedLesson, version, score, offline, online) {
              return AttendanceInfoBody(attendedLesson: attendedLesson, complitedLesson: complitedLesson, lessons: lessons, version: version, offline: offline, online: online, score: score, group: group, groupId: groupId);
            },
          );
        },
      ),
